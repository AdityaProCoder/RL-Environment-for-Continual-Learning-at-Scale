...
def run_experiment(cfg: ExperimentConfig, learner_names: List[str],
    ...
    for idx, name in enumerate(learner_names):
        ...
        with open(trajs_path, "w") as tf:
            while not env.done:
                prompt = learner.act_prompt(obs)
                raw = engine.generate(prompt, adapter_on=True)
                op = learner.decide(obs, None, False)
                obs2, reward, done, info = env.step(Action(answer=raw, learn_op=op))
        ...
        print(f"[GCL] [{name}] Finished in {round(time.time() - t0, 1)}s -> ACC: {acc:.3f} | BWT: {bwt:+.3f} | Forgetting: {forgetting:.3f} | Frontier: {frontier:+.4f}\n", flush=True)
...
def _write_summary(reports: Dict[str, Any], path: str) -> None:
    ...
    lines = [
        ...
        "", "| Learner | ACC ↑ | BWT ↑ | Forgetting ↓ | AUC ↑ | Stability | Updates | Rollbacks | Frontier ↑ |",
        "|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|",
    ]
    for name, d in reports.get("learners", {}).items():
        r = d["report"]
        lines.append(f"| {name} | {r['acc']:.3f} | {r['bwt']:+.3f} | {r['forgetting']:.3f} | {r['auc']:.3f} | {r['stability']:.3f} | {d['updates']} | {d['rollbacks']} | {d['frontier_score']:+.3f} |")
    lines.append("")
